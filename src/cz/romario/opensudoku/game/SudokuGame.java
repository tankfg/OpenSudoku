...
public class SudokuGame {
	...
	private final String TAG = "[SudokuGame]";
	...
	private boolean mAutoClearNotes = false;
	
	public boolean ismAutoClearNotes() {
		return mAutoClearNotes;
	}

	public void setmAutoClearNotes(boolean mAutoClearNotes) {
		this.mAutoClearNotes = mAutoClearNotes;
	}
	...
	public void setCellValue(Cell cell, int value) {
		...
		if (cell.isEditable()) {
			...
			{
				if(validate()) {
					if(ismAutoClearNotes())
						clearCellNote(cell, value);
				}
				
				//Log.i(TAG, "setCellValue Value:" + value);				
			}
	...
	public void clearCellNote(Cell cell, int value)
	{
		if (cell == null) {
			throw new IllegalArgumentException("Cell cannot be null.");
		}
		...
		//selCell.getNote().removeNumber(value);
		...
		{
			Cell tmpSector = sector.getCell(i);
			setCellNote(tmpSector, tmpSector.getNote().removeNumber(value));
			
			Cell tmpRow = row.getCell(i);
			setCellNote(tmpRow, tmpRow.getNote().removeNumber(value));

			Cell tmpCol =  column.getCell(i);
			setCellNote(tmpCol, tmpCol.getNote().removeNumber(value));
		}
	}
	/**
